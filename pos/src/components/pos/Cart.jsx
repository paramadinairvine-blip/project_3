...
export default function Cart() {
  ...
  const cartHeader = (
    <div className="flex items-center justify-between px-4 py-2.5 bg-gray-600 text-white">
      <h3 className="text-sm font-semibold">
        Daftar Pembelian
        {itemCount > 0 && (
          <span className="ml-1.5 text-xs font-normal text-gray-300">({itemCount} item)</span>
        )}
      </h3>
      <div className="flex items-center gap-2">
        ...
        {items.length > 0 && (
          <button
            onClick={clearCart}
            className="text-xs text-red-300 hover:text-red-100 flex items-center gap-1 transition-colors"
          >
